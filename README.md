...
- **Third-party script surface is minimal and intentional.** Plausible (analytics, cookieless, opt-in via an unset-by-default env var) and the lazy-loaded Paperform embed script (`join-paperform-embed.ts`, only injected once `/join`'s embed div nears the viewport) are the only third-party `<script>`s.
