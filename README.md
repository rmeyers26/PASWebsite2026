...
- **No dead code / no speculative abstraction.** Comments in the source explain _why_ a non-obvious decision was made (e.g. why `officers.photo` is a plain string instead of an `image()`-typed field), not what the code does — the code should already say that.
...
- **No secrets in the client bundle.** `NASA_API_KEY` is read via `import.meta.env` at **build time** only, and it isn't sensitive (a free, low-privilege rate-limit token). `.env` is git-ignored (`.gitignore`); `.env.example` documents the shape without real values.
...
- **No payment processing in this repo.** `join.astro` embeds the production Paperform membership form (`data-paperform-id="pasazmembership"`, the same form already live at pasaz.org) — Paperform hosts the form, collects submissions, and handles payment; no PayPal/Stripe SDK, API keys, or transaction handling exist in this codebase.
- **No PII collection in this repo.** Membership signup data goes straight to Paperform's hosted form, not to a first-party backend, since there is no backend.
- **Third-party script surface is minimal and intentional.** The lazy-loaded Paperform embed script (`join-paperform-embed.ts`, only injected once `/join`'s embed div nears the viewport) is the only third-party `<script>`.
