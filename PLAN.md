...
**Why this order:** Phase 1 deliberately front-loads the _visual_ transformation (the thing most visitors notice first) using only content that already exists, so PAS gets a fast, low-risk win. Each subsequent phase adds one category of complexity — content, then automation, then a new audience section, then accounts/gamification — rather than blocking launch on the hardest features (leaderboards, accounts) which are placed last on purpose.

_(No code has been written as part of this plan, per instruction — this is a content/design review and recommendation only.)_
